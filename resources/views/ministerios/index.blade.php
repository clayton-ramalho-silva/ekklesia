...
<section class="sessao">
    ...
    <article class="f-interna">
        ...
        <div class="table-container lista-curriculos">
            ...
            <ul class="tit-lista">                
                ...
                <li class="col-lider sortable" data-column="lider" data-type="text">Lider</li>                                             
            ...
            @if ($ministerios->count() > 0)
                ...
                <ul>                                       
                    ...
                    <li class="col-acoes d-flex">
                        <a href="{{ route('ministerios.edit', $ministerio) }}" target="_blanck">
                        ...
                        {{-- <a href="{{ route('ministerios.destroy', $ministerio) }}" onclick="return confirm('Tem certeza que deseja excluir este ministerio?')"> --}}
                        <a href="{{ route('ministerios.destroy', $ministerio) }}" onclick="event.preventDefault(); if(confirm('Tem certeza que deseja excluir este ministerio?')) { document.getElementById('delete-form-{{ $ministerio->id }}').submit(); }">
                            <form id="delete-form-{{ $ministerio->id }}" action="{{ route('ministerios.destroy', $ministerio) }}" method="POST" style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
            ...
            @else
            <span class="sem-resultado">Nenhum ministério encontrado</span>
            @endif
        ...
        <div class="pagination-wrapper">
            {{ $ministerios->appends(request()->query())->links('vendor.pagination.custom') }}
            <p class="pagination-info mt-3">Mostrando {{ $ministerios->firstItem() }} a {{ $ministerios->lastItem() }} de {{ $ministerios->total() }} currículos</p>
        </div>
    ...
    <article class="f4 bts-interna">
        <a href="{{ route('ministerios.create') }}" class="btInt btCadastrar">Cadastrar <small>Criar um novo ministério</small></a>
